defmodule Rep.LiftsTest do
  ...
  describe "mechanics" do
    ...
    @valid_attrs %{district: "some district"}
    @update_attrs %{district: "some updated district"}
    @invalid_attrs %{district: nil}

    def mechanic_fixture(attrs \\ %{}) do
      {:ok, mechanic} =
        attrs
        |> Enum.into(@valid_attrs)
        |> Lifts.create_mechanic()
    ...
    test "list_mechanics/0 returns all mechanics" do
      ...
      assert Lifts.list_mechanics() == [mechanic]
    end

    test "get_mechanic!/1 returns the mechanic with given id" do
      mechanic = mechanic_fixture()
      assert Lifts.get_mechanic!(mechanic.id) == mechanic
    end

    test "create_mechanic/1 with valid data creates a mechanic" do
      assert {:ok, %Mechanic{} = mechanic} = Lifts.create_mechanic(@valid_attrs)
      assert mechanic.district == "some district"
    end

    test "create_mechanic/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Lifts.create_mechanic(@invalid_attrs)
    end

    test "update_mechanic/2 with valid data updates the mechanic" do
      mechanic = mechanic_fixture()
      assert {:ok, mechanic} = Lifts.update_mechanic(mechanic, @update_attrs)
      assert %Mechanic{} = mechanic
      assert mechanic.district == "some updated district"
    end

    test "update_mechanic/2 with invalid data returns error changeset" do
      mechanic = mechanic_fixture()
      assert {:error, %Ecto.Changeset{}} = Lifts.update_mechanic(mechanic, @invalid_attrs)
      assert mechanic == Lifts.get_mechanic!(mechanic.id)
    end

    test "delete_mechanic/1 deletes the mechanic" do
      mechanic = mechanic_fixture()
      assert {:ok, %Mechanic{}} = Lifts.delete_mechanic(mechanic)
      assert_raise Ecto.NoResultsError, fn -> Lifts.get_mechanic!(mechanic.id) end
    end

    test "change_mechanic/1 returns a mechanic changeset" do
      mechanic = mechanic_fixture()
      assert %Ecto.Changeset{} = Lifts.change_mechanic(mechanic)
    end
  end
end
